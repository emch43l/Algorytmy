...
public class HashMap<Key, Value> : IEnumerable<Pair<Key, Value>>
{
    ...
    public bool PutIfAbsent(Key key, Value value)
    {
        ...
        if (_values[hashCode] == null)
            _values[hashCode] = new List<Pair<Key, Value>>();
    ...
    {
        int hashCode = HashCode(key);
        int index = _values[hashCode].FindIndex(x => x.Key.Equals(key));
    ...
    public bool Remove(Key key)
    {
        ...
        if (index == -1 || _values[hashCode][index] == null)
            return false;
        _values[hashCode].RemoveAt(index);
        _keys.Remove(key);
        return true;
    }
...
public class Product
{
    ...
    {
        return Math.Abs(Name.GetHashCode() + Price.GetHashCode() + Quantity.GetHashCode() + Unit.GetHashCode());
    }
}
